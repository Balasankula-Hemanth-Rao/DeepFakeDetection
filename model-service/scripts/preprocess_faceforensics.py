"""
FaceForensics Dataset Preprocessing Script

This script processes the downloaded FaceForensics dataset by:
1. Extracting frames from all videos at 3 FPS
...
Usage:
    python scripts/preprocess_faceforensics.py --input data/ --output data/processed/
    python scripts/preprocess_faceforensics.py --input data/ --output data/processed/ --max-videos 10 --debug
"""
...
def extract_frames_from_video(
    ...
    logger.debug(f"Extracting frames at {fps} FPS from: {video_path}")
...
def process_video(
    ...
    try:
        # Extract frames
        frame_count, _ = extract_frames_from_video(video_path, temp_dir, fps=fps)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Preprocess FaceForensics dataset for training',
...
Examples:
  ...
  python scripts/preprocess_faceforensics.py --input data/ --output data/processed/ --max-videos 10 --debug
    ...
    try:
        logger.info("=" * 70)
        logger.info("FaceForensics Preprocessing Started")
        logger.info("=" * 70)
